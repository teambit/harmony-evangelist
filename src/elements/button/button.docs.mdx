...
```js live=true
<Button onClick={() => new Promise((res) => setTimeout(res, 1500))} importance="ghost">
...
```js live=true
<>
	<Button onClick={() => new Promise((res) => setTimeout(res, 1000))} activeWhenLoading importance="ghost">
		click me repeatedly
	</Button>{' '}
	<Button onClick={() => new Promise((res) => setTimeout(res, 1000))} importance="ghost">
...
Setting a button as 'cta' indicates the user *should* press this button as the immediate next step.

> Do not add more than one cta button in a viewport, as this would confuse the user.

```js live
<ThemeCompositions style={{ minHeight: 90 }}>
  <Button importance="cta" style={{ width: 120 }}>Submit</Button>
</ThemeCompositions>
```
...
```js live
<ThemeCompositions style={{ minHeight: 90 }}>
  <Button importance="ghost" style={{ width: 120 }}>Update</Button>
</ThemeCompositions>
```
